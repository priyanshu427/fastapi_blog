...
from sqlalchemy import func, select, text  # select for querying
...
async def health_check(db: Annotated[AsyncSession, Depends(get_db)]):
    try:
        await db.execute(text("SELECT 1")) # very light query that checks if the db is reachable
...
async def home(request: Request, db: Annotated[AsyncSession, Depends(get_db)]):  # @app is used to define routes. @app tells Python this function belongs to your app.
    ...
    result = await db.execute(     # await freezes home function till the sqllite engine fetches the posts in the meanwhile takes other requests in the main event loop. when the query is done it pauses any other req and proceeds with the code in the function.
        ...
        .limit(settings.posts_per_page)    # setting limit using our settings
        )                         # no offset as this is gonna the first page
...
# Route for viewing  a specific users post(paginated)
...
async def user_posts_page(
    ...
    result = await db.execute(     # using the relationship to preload the sql alchemy object post,author
        select(models.Post)
        .options(selectinload(models.Post.author))
        .where(models.Post.user_id == user_id)
        .order_by(models.Post.date_posted.desc())   
...
async def reset_password_page(request: Request):
    ...
    response.headers["Referrer-Policy"] = "no-referrer"  # a security setting if the user on our page clicks a url the browser could send the url via refer header to the new site . the refer header has token which is a query parameter . we set refer header as no refer
    return response
