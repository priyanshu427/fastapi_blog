...
def generate_reset_token() -> str:   # the token that goes in the url.  secrets is hardware level random number generator . we takes 32 bytes and then we get 43-44 chars
    return secrets.token_urlsafe(32) # Standard random generation might produce characters like ? which have special meanings in a browser address bar and can break URLs. token_urlsafe guarantees the token only uses safe characters.

def hash_reset_token(token: str) -> str:  # sha gives  a64 character hexadecimal signature. encode converts chars to byte as sha is a low level func. hexdigest turns raw bytes back to 64 char string
    return hashlib.sha256(token.encode()).hexdigest()  # we are using sha256 instead of argon2 for token hash as user passwords are weak but our token are generated by server . also sha is faster
