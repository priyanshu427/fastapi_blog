...
# Copy app code and install project . if one line changes in code docker rebuilds the entire image and reinstalls all dependencies . by keeping dependencies above app code we prevent it from reinstalling dependencies when reconstructing image only the app code changes
COPY . ./
